...
func openTestConnConninfo(conninfo string) (*DB, error) {
	tStr1, tStr2 := convertStringFromGoToC("postgres", dsn)
	uptr := Open(tStr1, tStr2)
	db := GetDBInstance(uptr)
	if db == nil {
		return nil, errors.New("failed to test connection")
	}
	return db, nil
}

func openTestConn(t Fatalistic) *DB {
