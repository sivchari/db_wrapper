...
import (
	...
	"github.com/sivchari/database/oid"
)
...
func benchQueryLoop(b *testing.B, db *DB, query string, result interface{}) {
...
func ffakeConn(content string, prefixLen int) *conn {
...
func benchPreparedQueryLoop(b *testing.B, db *DB, stmt *Stmt, result interface{}) {
	rows, err := stmt.Query()
	if err != nil {
		b.Fatal(err)
	}
...
func BenchmarkMockPreparedSelectString(b *testing.B) {
	...
	const responses = parseResponse +
		...
		"C\x00\x00\x00\rSELECT 1\x00" +
		"Z\x00\x00\x00\x05I"
	c := ffakeConn(responses, len(parseResponse))
