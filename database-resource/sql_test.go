...
func TestOpenDB(t *testing.T) {
	uptr := openDB(dsnConnector{dsn: fakeDBName, driver: fdriver}, "", 1)
...
func TestDriverPanic(t *testing.T) {
	...
	expectPanic("Query Query", func() { db.QueryContext(context.Background(), "PANIC|Query|SELECT|people|age,name|") })
	expectPanic("Query NumInput", func() { db.QueryContext(context.Background(), "PANIC|NumInput|SELECT|people|age,name|") })
	expectPanic("Query Close", func() {
		rows, err := db.QueryContext(context.Background(), "PANIC|Close|SELECT|people|age,name|")
		if err != nil {
			t.Fatal(err)
		}
		rows.Close()
	})
	db.QueryContext(context.Background(), "PANIC|Exec|SELECT|people|age,name|") // should run successfully: Query does not call Exec
	exec(t, db, "WIPE")                                                         // check not deadlocked
}
...
func closeDB(t testing.TB, db *DB) {
	...
	db.mu.Unlock()
	utpr := uintptr(unsafe.Pointer(db))
	boolean := DBClose(utpr)
	if !boolean {
		t.Fatalf("error closing DB: %v", boolean)
	}
...
func TestQuery(t *testing.T) {
	...
	rows, err := db.QueryContext(context.Background(), "SELECT|people|age,name|")
	if err != nil {
...
func TestMultiResultSetQuery(t *testing.T) {
	...
	defer closeDB(t, db)
	prepares0 := numPrepares(t, db)
	rows, err := db.QueryContext(context.Background(), "SELECT|people|age,name|;SELECT|people|name|")
	if err != nil {
		t.Fatalf("Query: %v", err)
	}
	type row1 struct {
...
func TestTxRollbackCommitErr(t *testing.T) {
	...
	tuptr := Begin(uptr)
	boolean := Rollback(tuptr)
	if !boolean {
		t.Errorf("expected nil error from Rollback; got %v", boolean)
	}
	boolean = Commit(tuptr)
	if boolean {
		t.Errorf("expected %q from Commit; got %v", ErrTxDone, boolean)
	}
	...
	boolean = Commit(tuptr)
	if boolean {
		t.Errorf("expected nil error from Commit; got %v", boolean)
	}
	boolean = Rollback(tuptr)
	if boolean {
		t.Errorf("expected %q from Rollback; got %v", ErrTxDone, boolean)
	}
}
...
func TestStatementErrorAfterClose(t *testing.T) {
	db := newTestDB(t, "people")
	defer closeDB(t, db)
	stmt, err := db.PrepareContext(context.Background(), "SELECT|people|age|name=?")
	if err != nil {
...
func TestBogusPreboundParameters(t *testing.T) {
	...
	_, err := db.PrepareContext(context.Background(), "INSERT|t1|name=?,age=bogusconversion")
	if err == nil {
...
func TestSQLExec(t *testing.T) {
	...
	defer closeDB(t, db)
	exec(t, db, "CREATE|t1|name=string,age=int32,dead=bool")
	stmt, err := db.PrepareContext(context.Background(), "INSERT|t1|name=?,age=?")
	if err != nil {
	...
	for n, et := range execTests {
		_, err := stmt.ExecContext(context.Background(), et.args...)
		errStr := ""
		if err != nil {
			errStr = err.Error()
		}
...
func TestTxPrepare(t *testing.T) {
	...
	uptr := uintptr(unsafe.Pointer(db))
	tuptr := Begin(uptr)
	tx := GetTxInstance(tuptr)
	stmt, err := tx.PrepareContext(context.Background(), "INSERT|t1|name=?,age=?")
	if err != nil {
		t.Fatalf("Stmt, err = %v, %v", stmt, err)
	}
	defer stmt.Close()
	_, err = stmt.ExecContext(context.Background(), "Bobby", 7)
	if err != nil {
		t.Fatalf("Exec = %v", err)
	}
	boolean := Commit(tuptr)
	if !boolean {
		t.Fatalf("Commit = %v", boolean)
	}
...
func TestTxStmt(t *testing.T) {
	...
	txs := tx.Stmt(stmt)
	defer txs.Close()
	_, err = txs.ExecContext(context.Background(), "Bobby", 7)
	if err != nil {
		t.Fatalf("Exec = %v", err)
	}
	boolean := Commit(tuptr)
...
func TestTxStmtPreparedOnce(t *testing.T) {
	...
	_, err = txs1.ExecContext(context.Background(), "Go", 7)
	if err != nil {
		t.Fatalf("Exec = %v", err)
	}
	txs1.Close()

	_, err = txs2.ExecContext(context.Background(), "Gopher", 8)
	if err != nil {
		t.Fatalf("Exec = %v", err)
	}
	txs2.Close()
...
func TestTxStmtClosedRePrepares(t *testing.T) {
	...
	_, err = txs.ExecContext(context.Background(), `Eric`, 82)
	if err != nil {
...
func TestParentStmtOutlivesTxStmt(t *testing.T) {
	...
	_, err = txs.ExecContext(context.Background(), "Suzan", 30)
	if err == nil {
		t.Fatalf("txs.Exec(), expected err")
	}
	// Stmt must still be valid.
	_, err = stmt.ExecContext(context.Background(), "Janina", 25)
	if err != nil {
...
func TestTxStmtFromTxStmtRePrepares(t *testing.T) {
	...
	_, err = txs2.ExecContext(context.Background(), `Eric`, 82)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTxQuery(t *testing.T) {
	...
	tuptr := Begin(uptr)
	tx := GetTxInstance(tuptr)
	defer Rollback(tuptr)

	r, err := tx.QueryContext(context.Background(), "SELECT|t1|name|")
	if err != nil {
		t.Fatal(err)
	}
	defer r.Close()
...
func TestTxQueryInvalid(t *testing.T) {
	...
	_, err := tx.QueryContext(context.Background(), "SELECT|t1|name|")
	if err == nil {
...
func TestIssue2542Deadlock(t *testing.T) {
	...
	closeDB(t, db)
	for i := 0; i < 2; i++ {
		_, err := db.QueryContext(context.Background(), "SELECT|people|age,name|")
		if err == nil {
			t.Fatalf("expected error")
		}
	}
...
func TestSQLCloseStmtBeforeRows(t *testing.T) {
	...
	s, err := db.PrepareContext(context.Background(), "SELECT|people|name|")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNullTimeParam(t *testing.T) {
	...
	spec := nullTestSpec{"nulldatetime", "datetime", [6]nullTestRow{
		{sql.NullTime{t1, true}, t2, sql.NullTime{t1, true}},
		{sql.NullTime{t1, false}, t2, sql.NullTime{t0, false}},
		{t1, t2, sql.NullTime{t1, true}},
		{sql.NullTime{t1, true}, t2, sql.NullTime{t1, true}},
		{sql.NullTime{t1, false}, t2, sql.NullTime{t0, false}},
		{t2, sql.NullTime{t1, false}, nil},
	}}
	nullTestRun(t, spec)
}
...
func nullTestRun(t *testing.T, spec nullTestSpec) {
	...
	stmt, err := db.PrepareContext(context.Background(), "INSERT|t|id=?,name=?,nullf=?,notnullf=?")
	if err != nil {
		t.Fatalf("prepare: %v", err)
	}
	defer stmt.Close()
	if _, err := stmt.ExecContext(context.Background(), 3, "chris", spec.rows[2].nullParam, spec.rows[2].notNullParam); err != nil {
		t.Errorf("exec insert chris: %v", err)
	}
	if _, err := stmt.ExecContext(context.Background(), 4, "dave", spec.rows[3].nullParam, spec.rows[3].notNullParam); err != nil {
		t.Errorf("exec insert dave: %v", err)
	}
	if _, err := stmt.ExecContext(context.Background(), 5, "eleanor", spec.rows[4].nullParam, spec.rows[4].notNullParam); err != nil {
		t.Errorf("exec insert eleanor: %v", err)
	}

	// Can't put null val into non-null col
	if _, err := stmt.ExecContext(context.Background(), 6, "bob", spec.rows[5].nullParam, spec.rows[5].notNullParam); err == nil {
...
func TestIssue4902(t *testing.T) {
	...
	var err error
	for i := 0; i < 10; i++ {
		stmt, err = db.PrepareContext(context.Background(), "SELECT|people|name|")
		if err != nil {
			t.Fatal(err)
		}
		err = stmt.Close()
...
func TestSimultaneousQueries(t *testing.T) {
	...
	r1, err := tx.QueryContext(context.Background(), "SELECT|people|name|")
	if err != nil {
		t.Fatal(err)
	}
	defer r1.Close()

	r2, err := tx.QueryContext(context.Background(), "SELECT|people|name|")
	if err != nil {
		t.Fatal(err)
	}
	defer r2.Close()
...
func TestMaxOpenConns(t *testing.T) {
	...
	stmt, err := db.PrepareContext(context.Background(), "SELECT|magicquery|op|op=?,millis=?")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSingleOpenConn(t *testing.T) {
	...
	rows, err := db.QueryContext(context.Background(), "SELECT|people|name|")
	if err != nil {
		t.Fatal(err)
	}
	if err = rows.Close(); err != nil {
...
func TestStmtCloseOrder(t *testing.T) {
	...
	_, err := db.QueryContext(context.Background(), "SELECT|non_existent|name|")
	if err == nil {
...
func TestManyErrBadConn(t *testing.T) {
	...
	uptr := uintptr(unsafe.Pointer(db))
	tuptr := Begin(uptr)
	if boolean := Rollback(tuptr); !boolean {
		t.Fatal(err)
	}
	...
	db = manyErrBadConnSetup(func(db *DB) {
		stmt, err = db.PrepareContext(context.Background(), "INSERT|people|name=Julia,age=19")
		if err != nil {
			t.Fatal(err)
		}
	})
...
func TestTxCannotCommitAfterRollback(t *testing.T) {
	...
	tuptr := uintptr(unsafe.Pointer(tx))
...
func TestTxStmtDeadlock(t *testing.T) {
	...
	stmt, err := tx.PrepareContext(ctx, "SELECT|people|name,age|age=?")
	if err != nil {
		t.Fatal(err)
	}
...
func TestIssue20622(t *testing.T) {
	...
	rows, err := tx.QueryContext(ctx, "SELECT|people|age,name|")
	if err != nil {
		t.Fatal(err)
	}
...
func TestErrBadConnReconnect(t *testing.T) {
	...
	// db.Query
	dbQuery := func() error {
		rows, err := db.QueryContext(context.Background(), "SELECT|t1|age,name|")
		if err == nil {
			err = rows.Close()
		}
		return err
	...
	simulateBadConn("db.Prepare", &hookPrepareBadConn, func() error {
		stmt, err := db.PrepareContext(context.Background(), "INSERT|t1|name=?,age=?,dead=?")
		if err != nil {
			return err
		}
		stmt.Close()
	...
	// stmt.Exec
	stmt1, err := db.PrepareContext(context.Background(), "INSERT|t1|name=?,age=?,dead=?")
	if err != nil {
		t.Fatalf("prepare: %v", err)
	}
	defer stmt1.Close()
	...
	stmtExec := func() error {
		_, err := stmt1.ExecContext(context.Background(), "Gopher", 3, false)
		return err
	}
	...
	// stmt.Query
	stmt2, err := db.PrepareContext(context.Background(), "SELECT|t1|age,name|")
	if err != nil {
		t.Fatalf("prepare: %v", err)
	}
	defer stmt2.Close()
...
// 	// 		tuptr := Begin(uptr)
// 	// 		tx := GetTxInstance(tuptr)
// 	// 		_, err := tx.ExecContext(context.Background(), "INSERT|t1|name=?,age=?,dead=?", "Gordon", 3, true)
// 	// 		if err != nil {
// 	// 			return err
// 	// 		}
// 	// 		return endTx(tx)
...
func (c *concurrentDBQueryTest) test(t testing.TB) error {
	rows, err := c.db.QueryContext(context.Background(), "SELECT|people|name|")
	if err != nil {
		t.Error(err)
		return err
	}
...
func (c *concurrentStmtQueryTest) init(t testing.TB, db *DB) {
	c.db = db
	var err error
	c.stmt, err = db.PrepareContext(context.Background(), "SELECT|people|name|")
	if err != nil {
		t.Fatal(err)
	}
}
...
func (c *concurrentStmtExecTest) init(t testing.TB, db *DB) {
	c.db = db
	var err error
	c.stmt, err = db.PrepareContext(context.Background(), "NOSERT|people|name=Chris,age=?,photo=CPHOTO,bdate=?")
	if err != nil {
		t.Fatal(err)
	}
}
...
func (c *concurrentStmtExecTest) test(t testing.TB) error {
	_, err := c.stmt.ExecContext(context.Background(), 3, chrisBirthday)
	if err != nil {
...
func (c *concurrentTxQueryTest) init(t testing.TB, db *DB) {
	c.db = db
	uptr := uintptr(unsafe.Pointer(c.db))
	tuptr := Begin(uptr)
	tx := GetTxInstance(tuptr)
	c.tx = tx
}
...
func (c *concurrentTxQueryTest) finish(t testing.TB) {
	tuptr := uintptr(unsafe.Pointer(c.tx))
	if c.tx != nil {
		Rollback(tuptr)
		c.tx = nil
	}
...
func (c *concurrentTxStmtQueryTest) init(t testing.TB, db *DB) {
	...
	tx := GetTxInstance(tuptr)
	c.tx = tx
	c.stmt, err = c.tx.PrepareContext(context.Background(), "SELECT|people|name|")
	if err != nil {
		t.Fatal(err)
	}
}
...
func (c *concurrentTxStmtExecTest) init(t testing.TB, db *DB) {
	...
	c.tx = tx
	var err error
	c.stmt, err = c.tx.PrepareContext(context.Background(), "NOSERT|people|name=Chris,age=?,photo=CPHOTO,bdate=?")
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestConnectionLeak(t *testing.T) {
	...
	for ii := range rows {
		r, err := db.QueryContext(context.Background(), "SELECT|people|name|")
		if err != nil {
			t.Fatal(err)
		}
		r.Next()
...
func TestOpenConnector(t *testing.T) {
	...
	DBClose(uptr)
	if boolean := DBClose(uptr); !boolean {
		t.Fatal(boolean)
	}
...
func TestRowsScanProperlyWrapsErrors(t *testing.T) {
	...
	rows, err := db.QueryContext(context.Background(), "SELECT|people|age|")
	if err != nil {
		t.Fatalf("Query: %v", err)
	}
...
func TestSQLPing(t *testing.T) {
	...
	if boolean := Ping(uptr); boolean != true {
		t.Errorf("err was %#v, expected nil", boolean)
		return
	}

	driver.fails = true
	if boolean := Ping(uptr); boolean != false {
		t.Errorf("err was %#v, expected pingError", boolean)
	}
}
