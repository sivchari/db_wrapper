...
type DB struct {
	...
	// add this line for switch sql client
	driverName string
}
...
type cDriverName = *C.char

// GetDBDriverName gets the driver name
//nolint:govet, golint
//export GetDBDriverName
func GetDBDriverName(utpr uintptr) cDriverName {
	db := GetDBInstance(utpr)
	driver := C.CString(db.driverName)
...
func openDB(c driver.Connector, driverName string, connectionPool int) uintptr {
...
// close a DB.
//export DBOpen
func DBOpen(cDriverName, cDataSourceName *C.char, connectionPool int) uintptr {
	driverName := C.GoString(cDriverName)
	dataSourceName := C.GoString(cDataSourceName)
	driversMu.RLock()
	register(driverName)
	...
	if driverCtx, ok := driveri.(driver.DriverContext); ok {
		...
		return openDB(connector, driverName, connectionPool)
	}
	return openDB(dsnConnector{dsn: dataSourceName, driver: driveri}, driverName, connectionPool)
}

const _mysql = "mysql"
const _postgres = "postgres"
const _sqlite3 = "sqlite3"
...
func register(driverName string) {
	...
	case _sqlite3:
		drivers[driverName] = &sqlite3.SQLiteDriver{}
	default:
	}
}
...
//export DBClose
func DBClose(uptr uintptr) bool {
	db := GetDBInstance(uptr)
	db.mu.Lock()
...
// CRows is the result of a query
type CRows struct {
	columns     []string
	rows        []interface{}
	types       []string
...
var RowsInstances map[uintptr]*CRows

// GetRowsInstance is getter method getting DB struct by uintptr
func GetRowsInstance(uptr uintptr) *CRows {
	rows := (*CRows)(unsafe.Pointer(uptr))
	if rows == nil {
...
type ccolumns = **C.char

// GetColumns gets row column's names
//nolint:govet, golint
//export GetColumns
func GetColumns(uptr uintptr) ccolumns {
	rows := GetRowsInstance(uptr)
	cMalloc := C.malloc(C.size_t(unsafe.Sizeof(len(rows.columns))) * C.size_t(unsafe.Sizeof(uintptr(0))))
	cArray := (*[C.size_t(unsafe.Sizeof(len(rows.columns))) * C.size_t(unsafe.Sizeof(uintptr(0)))]*C.char)(cMalloc)
	for i, v := range rows.columns {
		cArray[i] = C.CString(v)
	}
	// defer C.free(unsafe.Pointer(cMalloc))
	return (**C.char)(cMalloc)
}
...
//nolint:govet, golint
//export GetRow
func GetRow(uptr uintptr, i int) crow {
	rows := GetRowsInstance(uptr)
	row := (rows.rows)[i].([]interface{})
	cMalloc := C.malloc(C.size_t(unsafe.Sizeof(len(row))) * C.size_t(unsafe.Sizeof(uintptr(0))))
	// defer C.free(unsafe.Pointer(cMalloc))
	cArray := (*[C.size_t(unsafe.Sizeof(len(row))) * C.size_t(unsafe.Sizeof(uintptr(0)))]*C.char)(cMalloc)
	for idx, v := range row {
		switch v := v.(type) {
		case int:
			vv := strconv.Itoa(v)
...
//nolint:govet, golint
//export GetTypes
func GetTypes(uptr uintptr) ctypes {
	rows := GetRowsInstance(uptr)
	cMalloc := C.malloc(C.size_t(unsafe.Sizeof(len(rows.types))) * C.size_t(unsafe.Sizeof(uintptr(0))))
	// defer C.free(unsafe.Pointer(cMalloc))
	cArray := (*[C.size_t(unsafe.Sizeof(len(rows.types))) * C.size_t(unsafe.Sizeof(uintptr(0)))]*C.char)(cMalloc)
	for idx, v := range rows.types {
		cArray[idx] = C.CString(v)
	}
	return (**C.char)(cMalloc)
}
...
//nolint:govet, golint
//export GetRowsCount
func GetRowsCount(uptr uintptr) ccount {
	rows := GetRowsInstance(uptr)
	return C.int(rows.rowsCount)
}

// GetCount get count of query result
//nolint:govet, golint
//export GetColumnCount
func GetColumnCount(uptr uintptr) ccount {
	rows := GetRowsInstance(uptr)
	return C.int(rows.columnCount)
}
...
// TxInstances ptr instances
var TxInstances map[uintptr]*Tx

// GetTxInstance is getter method getting DB struct by uintptr
func GetTxInstance(uptr uintptr) *Tx {
	tx := (*Tx)(unsafe.Pointer(uptr))
	if tx == nil {
		log.Fatalf("Tx instance is nil: %v", tx)
	}
	return tx
}

type ctxDriverName = *C.char

// GetTxDriverName is getter method getting DB struct by uintptr
//nolint:govet, golint
//export GetTxDriverName
func GetTxDriverName(utpr uintptr) ctxDriverName {
	tx := GetTxInstance(utpr)
	driver := C.CString(tx.driverName)
	// defer C.free(unsafe.Pointer(driver))
	return driver
}
...
func Begin(u uintptr) uintptr {
	...
	uptr := uintptr(unsafe.Pointer(tx))
	if TxInstances == nil {
		TxInstances = make(map[uintptr]*Tx)
	}
...
//export Commit
func Commit(uptr uintptr) bool {
	// Check context first to avoid transaction leak.
	// If put it behind tx.done CompareAndSwap statement, we can't ensure
	// the consistency between tx.done and the real COMMIT operation.
	tx := GetTxInstance(uptr)
	select {
	default:
	case <-tx.ctx.Done():
		if atomic.LoadInt32(&tx.done) == 1 {
			log.Println(ErrTxDone)
			return false
		}
		log.Println(tx.ctx.Err())
		return false
	}
...
//export Rollback
func Rollback(uptr uintptr) bool {
	tx := GetTxInstance(uptr)
...
// PrepareContext.
//export TxPrepare
func TxPrepare(u uintptr, cQuery *C.char) uintptr {
	query := C.GoString(cQuery)
	tx := GetTxInstance(u)
...
// QueryContext.
//export TxQueryExec
func TxQueryExec(u uintptr, cQuery *C.char) uintptr {
	query := C.GoString(cQuery)
	tx := GetTxInstance(u)
...
var rowsSync sync.Mutex

func convertTocRows(rows *Rows) uintptr {
	...
	crows := &CRows{
		...
		rowsCount:   len(results),
	}

	uptr := uintptr(unsafe.Pointer(crows))
	if RowsInstances == nil {
		RowsInstances = make(map[uintptr]*CRows)
	}
	rowsSync.Lock()
	RowsInstances[uptr] = crows
	rowsSync.Unlock()
