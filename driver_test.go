...
func init() {
	...
	uuser = env("MYSQL_TEST_USER", "root")
	pass = env("MYSQL_TEST_PASS", "")
	prot = env("MYSQL_TEST_PROT", "tcp")
	addr = env("MYSQL_TEST_ADDR", "localhost:3306")
	dbname = env("MYSQL_TEST_DBNAME", "gotest")
	netAddr = fmt.Sprintf("%s(%s)", prot, addr)
	dsn = fmt.Sprintf("%s:%s@%s/%s?timeout=30s", uuser, pass, netAddr, dbname)
...
func runTestsWithMultiStatement(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	if _, err := ParseDSN(dsn); err != errInvalidDSNUnsafeCollation {
		tStr1, tStr2 := convertStringFromGoToC("mysql", dsn)
		uptr := Open(tStr1, tStr2)
		db := GetDBInstance(uptr)
		if db == nil {
			t.Fatalf("error connecting: %p", db)
		}
		defer db.Close()
	}
...
func runTests(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	if _, err := ParseDSN(dsn2); err != errInvalidDSNUnsafeCollation {
		tStr1, tStr2 := convertStringFromGoToC("mysql", dsn)
		uptr := Open(tStr1, tStr2)
		db2 := GetDBInstance(uptr)
		if db2 == nil {
			t.Fatalf("error connecting: %p", db2)
		}
		defer db2.Close()
	}
	...
	if _, err := ParseDSN(dsn3); err != errInvalidDSNUnsafeCollation {
		tStr1, tStr2 := convertStringFromGoToC("mysql", dsn)
		uptr := Open(tStr1, tStr2)
		db3 = GetDBInstance(uptr)
		if db3 == nil {
			t.Fatalf("error connecting: %p", db3)
		}
		if err != nil {
...
func TestDriverCloseStmtBeforeRows(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
...
func testDialError(t *testing.T, dialErr error, expectErr error) {
	...
	tStr1, tStr2 := convertStringFromGoToC("mysql", fmt.Sprintf("%s:%s@mydial(%s)/%s?timeout=30s", uuser, pass, addr, dbname))
	uptr := Open(tStr1, tStr2)
	db := GetDBInstance(uptr)
	if db == nil {
		t.Fatalf("error connecting: %p", db)
...
func TestUnixSocketAuthFail(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		// badDSN := fmt.Sprintf("%s:%s@unix(%s)/%s?timeout=30s", uuser, badPass, socket, dbname)
		tStr1, tStr2 := convertStringFromGoToC("mysql", fmt.Sprintf("%s:%s@mydial(%s)/%s?timeout=30s", uuser, pass, addr, dbname))
		uptr := Open(tStr1, tStr2)
		db := GetDBInstance(uptr)
		if db == nil {
...
func TestMultiResultSet(t *testing.T) {
	...
	checkRows := func(desc string, rows *Rows, dbt *DBTest) {
		expected := []result{
			{
				values:  [][]int{{1, 2}, {3, 4}},
...
func TestDriverContextCancelExec(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		dbt.mustExec("CREATE TABLE test (v INTEGER)")
		ctx, cancel := context.WithCancel(context.Background())
...
func TestDriverContextCancelQuery(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		dbt.mustExec("CREATE TABLE test (v INTEGER)")
		ctx, cancel := context.WithCancel(context.Background())
...
func TestDriverContextCancelBegin(t *testing.T) {
	...
	runTests(t, dsn, func(dbt *DBTest) {
		...
		// is cancelled
		_, err = conn.BeginTx(context.Background(), &TxOptions{Isolation: LevelReadCommitted})
...
func TestContextBeginIsolationLevel(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		tx1, err := dbt.db.BeginTx(ctx, &TxOptions{
			Isolation: LevelRepeatableRead,
		})
		if err != nil {
			dbt.Fatal(err)
		}

		tx2, err := dbt.db.BeginTx(ctx, &TxOptions{
			Isolation: LevelReadCommitted,
		})
		if err != nil {
			dbt.Fatal(err)
		}
...
func TestContextBeginReadOnly(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		tx, err := dbt.db.BeginTx(ctx, &TxOptions{
			ReadOnly: true,
		})
...
func TestDriverRowsColumnTypes(t *testing.T) {
	niNULL := NullInt64{Int64: 0, Valid: false}
	ni0 := NullInt64{Int64: 0, Valid: true}
	ni1 := NullInt64{Int64: 1, Valid: true}
	ni42 := NullInt64{Int64: 42, Valid: true}
	nfNULL := NullFloat64{Float64: 0.0, Valid: false}
	nf0 := NullFloat64{Float64: 0.0, Valid: true}
	nf1337 := NullFloat64{Float64: 13.37, Valid: true}
	...
	rb0 := RawBytes("0")
	rb42 := RawBytes("42")
	rbTest := RawBytes("Test")
	rb0pad4 := RawBytes("0\x00\x00\x00") // BINARY right-pads values with 0x00
	rbx0 := RawBytes("\x00")
	rbx42 := RawBytes("\x42")
	...
	var columns = []struct {
		...
		{"decimal1", "DECIMAL(10,6) NOT NULL", "DECIMAL", scanTypeRawBytes, false, 10, 6, [3]string{"0", "13.37", "1234.123456"}, [3]interface{}{RawBytes("0.000000"), RawBytes("13.370000"), RawBytes("1234.123456")}},
		{"decimal1null", "DECIMAL(10,6)", "DECIMAL", scanTypeRawBytes, true, 10, 6, [3]string{"0", "NULL", "1234.123456"}, [3]interface{}{RawBytes("0.000000"), rbNULL, RawBytes("1234.123456")}},
		{"decimal2", "DECIMAL(8,4) NOT NULL", "DECIMAL", scanTypeRawBytes, false, 8, 4, [3]string{"0", "13.37", "1234.123456"}, [3]interface{}{RawBytes("0.0000"), RawBytes("13.3700"), RawBytes("1234.1235")}},
		{"decimal2null", "DECIMAL(8,4)", "DECIMAL", scanTypeRawBytes, true, 8, 4, [3]string{"0", "NULL", "1234.123456"}, [3]interface{}{RawBytes("0.0000"), rbNULL, RawBytes("1234.1235")}},
		{"decimal3", "DECIMAL(5,0) NOT NULL", "DECIMAL", scanTypeRawBytes, false, 5, 0, [3]string{"0", "13.37", "-12345.123456"}, [3]interface{}{rb0, RawBytes("13"), RawBytes("-12345")}},
		{"decimal3null", "DECIMAL(5,0)", "DECIMAL", scanTypeRawBytes, true, 5, 0, [3]string{"0", "NULL", "-12345.123456"}, [3]interface{}{rb0, rbNULL, RawBytes("-12345")}},
			...
			for j := range values {
				...
				if !reflect.DeepEqual(value, columns[j].valuesOut[i]) {
					if columns[j].scanType == scanTypeRawBytes {
						t.Errorf("row %d, column %d: %v != %v", i, j, string(value.(RawBytes)), string(columns[j].valuesOut[i].(RawBytes)))
					} else {
		...
		for i := 0; i < contextRaceIterations; i++ {
			func() {
				...
				for rows.Next() {
					...
					if before != after {
						t.Fatalf("the backing storage for RawBytes has been modified (i=%v)", i)
					}
				}
				rows.Close()
			}()
